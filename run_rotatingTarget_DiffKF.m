...
for i=1:nm.getNumNodes()
    ...
    nm.nodes{i}.P = [175,0;0,175];
    nm.nodes{i}.x = [0;0];
    nm.nodes{i}.x_zonotope = zonotope([nm.nodes{i}.x,diag([3*sqrt(nm.nodes{i}.P(1,1)),3*sqrt(nm.nodes{i}.P(2,2))])]);
    nid = node_ids(i);
...
while (t_last - t_start) < t_stop
    ...
    nm.checkekf_p2_forall(f,Q ,diffEnable);
...
%save('cache/DKF/DKF_2neigh', 'nm', 'k', 'p_history','targetLoc_history' ,'t_history','pSupremumAll_history','pInfimumAll_history','disList');
%sync_history(end,:)
